...
block_matrices_solve <- function(Klist, Glist, Pmatrix,
  ...
  p <- fftw::planFFT(max.offspring+1)
  pad.mat <- matrix(0, nrow = max.offspring - dim(Klist[[1]])[1]+1, ncol = n.stages)
  ...
  betahat.mat <-matrix(0, nrow = max.offspring+1, ncol = n.stages*n.ages)
  for (k in 1:(max.offspring+1)) {
    wvec <- c()
    dhatvec <- c()
    if(n.ages>1){
      ...
      Q <- t(Glist[[n.ages]] %*% diag(Pmatrix))#diag(Pmatrix[,n.ages]))
      C <- I - Q %*% W
      ...
      wvec <- kappahat.mat[[n.ages]][k,]
      W <- diag(wvec)
      Q <- t(Glist[[n.ages]] %*% diag(Pmatrix))
      C <- I - Q %*% W
      d.hat <- solve(C) %*% d
      dhatvec <- d.hat
      #betahat.mat[k,] <- diag(wvec) %*% dhatvec # element products, as.vector(d.hat) read matrix from colunm to colunm
  ...
  colnames(gamma) <- paste0("stage", seq_len(ncol(gamma))) #stage here is a general term, it can be stage, or age X stage, or env X age X stge
  return(gamma)
}

block_matrices_solve0 <- function(Klist, Glist, Pmatrix,
                                 max.offspring = 2^4,
                                 initial.age = 1,
                                 end.age = 1,
                                 stage_per_age){
  ...
  #write.table(gamma, paste("age", (end.age- initial.age + 1), "X", stage_per_age, "stages.roedeer.block_fft", max.offspring,".txt", sep = ""), quote = F, row.names = F, col.names = F)
  return(gamma)# the output is env X (age X (stage))
}
